...
void AIslandsManager::BuildFromData(const TArray<FIslandDTO>& Islands)
{
    ...
    {
		FIslandInstanceData Data;
		Data.Center = Island.Pos;
		Data.CurrentYaw = 0.f;
        ...
        if (bUseBaseIslands && BaseMesh)
        {
            ...
            Data.BaseIndex = BaseHISM->AddInstance(BaseT);
        }
        ...
        for (int32 i = 0; i < 5 && i < Island.Bars.Num(); ++i)
        {
            ...
            {
                Data.BarHeights[i] = H;
                Data.BarIndices[i] = BarComps[i]->AddInstance(T);
            }
        }
...
void AIslandsManager::Tick(float DeltaTime)
{
    ...
    for (FIslandInstanceData& IslandData : SpawnedIslands)
    {
        ...
        const FRotator Rot(0.f, IslandData.CurrentYaw, 0.f);

        if (IslandData.BaseIndex != INDEX_NONE)
        {
            const FTransform BaseT(Rot, IslandData.Center, BaseScale);
            BaseHISM->UpdateInstanceTransform(IslandData.BaseIndex, BaseT, false, false, true);
        }

        for (int32 i = 0; i < 5; ++i)
        {
            if (IslandData.BarIndices[i] == INDEX_NONE || !BarComps[i])
                continue;

            const float H = IslandData.BarHeights[i];
            const FVector RotatedOffset = Rot.RotateVector(Offsets[i]);
            const FVector Loc = IslandData.Center + RotatedOffset + FVector(0.f, 0.f, H * 0.5f);
            const FVector Scale(BarXYScale, BarXYScale, H / FMath::Max(1.f, BarMeshHeightCM));
            const FTransform T(Rot, Loc, Scale);

            BarComps[i]->UpdateInstanceTransform(IslandData.BarIndices[i], T, false, false, true);
        }
    }

    BaseHISM->MarkRenderStateDirty();

    for (int32 i = 0; i < 5; ++i)
    {
        if (BarComps[i])
        {
            BarComps[i]->MarkRenderStateDirty();
        }
    }
}